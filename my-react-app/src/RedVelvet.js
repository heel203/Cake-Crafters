...
function RedVelvetCakeProduct({ id, imageUrl, name, price, addToCart }) {
    return (
...
function RedVelvetCakeSection() {
    const [products, setProducts] = useState([]);

    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:4001/api/products/redVelvet');
    ...
    return (
        <div className="container-fluid mt-5 mb-5" id="redvalvet">
            <div className="heading-line text-center mb-5" data-aos="zoom-in">
                <h1 className="heading-up text-center font-weight p-1 font-cursive">Red Velvet Cake</h1>
            </div>
